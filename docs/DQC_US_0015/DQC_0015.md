...
The US GAAP Taxonomy is designed so that the majority of elements have a positive value. This rules tests whether the values for a given list of elements are negative values. In some cases, if the value also has a specified member associated with it, the value may be negative. The rule excludes these elements when these members are applied.  

See file DQC_0015_List_Of_Elements - which provides a list of elements that should not be negative. The rule tests whether these elements have a negative value in an instance file.  The elements are listed in order of most commonly occurring errors. 

See file DQC_0015_Member_Exclusions for a list of members that, when applied to these elements, allow the value of the element to be negative.  The rule does not fire for these member/element combinations.
...
If the value is shown with brackets in the HTML filing, the value should still be entered as a positive number, and a negating label can be used which will present the value in brackets in the rendering of the XBRL file. Because negating labels will not change the XBRL value in the filing, the SEC rendered version of the filing should not be used to determine whether the value was entered correctly.
...
Accrued Liabilities And Other Liabilities has a value of -400,000 which is less than zero. This element should not have a negative value, except in rare cases where the value is immaterial. 
...
See DQC_0015_List_Of_Elements and DQC_0015_Member_Exclusions spreadsheets for rule element ids.
...
### DQC_0015.2378 (Selling, General and Administrative Expense)

${fact1.label} has a value of ${fact1.value} which is less than zero. In those cases where the value is represented as a reclassification out of AOCI the value should be entered as a positive amount to represent the impact on the element ${fact1.label} and not on AOCI. 
The properties of this ${fact1.name} fact are:
Period: ${fact1.period}
Dimensions: ${fact1.dimensions}
Unit: ${fact1.unit}
